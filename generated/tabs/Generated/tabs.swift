...
fileprivate extension RustBuffer {
    ...
    init(bytes: [UInt8]) {
        let rbuf = bytes.withUnsafeBufferPointer { ptr in
            try! rustCall { ffi_tabs_beeb_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
        }
    ...
    func deallocate() {
        try! rustCall { ffi_tabs_beeb_rustbuffer_free(self, $0) }
    }
}
...
extension String: ViaFfi {
    ...
    fileprivate static func lift(_ v: FfiType) throws -> Self {
        defer {
            try! rustCall { ffi_tabs_beeb_rustbuffer_free(v, $0) }
        }
        if v.data == nil {
            return String()
        }
    ...
    fileprivate func lower() -> FfiType {
        return self.utf8CString.withUnsafeBufferPointer { ptr in
            ...
            ptr.withMemoryRebound(to: UInt8.self) { ptr in
                ...
                return try! rustCall { ffi_tabs_beeb_rustbuffer_from_bytes(bytes, $0) }
            }
        }
    }
...
fileprivate extension RemoteTab {
    static func read(from buf: Reader) throws -> RemoteTab {
        return try RemoteTab(
            ...
            urlHistory: [String].read(from: buf),
            icon: String?.read(from: buf),
            lastUsed: Int64.read(from: buf)
        )
    }

    func write(into buf: Writer) {
        title.write(into: buf)
        urlHistory.write(into: buf)
        icon.write(into: buf)
        lastUsed.write(into: buf)
    }
}

extension RemoteTab: ViaFfiUsingByteBuffer, ViaFfi {}
...
fileprivate extension ClientRemoteTabs {
    static func read(from buf: Reader) throws -> ClientRemoteTabs {
        return try ClientRemoteTabs(
            ...
            clientName: String.read(from: buf),
            deviceType: DeviceType.read(from: buf),
            remoteTabs: [RemoteTab].read(from: buf)
        )
    }

    func write(into buf: Writer) {
        clientId.write(into: buf)
        clientName.write(into: buf)
        deviceType.write(into: buf)
        remoteTabs.write(into: buf)
    }
}
